...
public class Socket {
	...
	public void send(Vector<Integer> data) throws IOException {
		...
		// System.err.println ("Send " + numBytes + " bytes via tcpip::socket: "
		// + data.toString());
	...
	public void sendExact(Storage storageToSend) throws IOException {
		 ...
		 * for (int i = 0; i < length; ++i) System.err.print( " " + buffer[i] +
		 * " "); System.err.println("]");
		 */
		outStream.write(buffer);
	}

	public Vector<Integer> receive(int bufSize) throws UnknownHostException,
			IOException {
		...
		if (bytesRead == -1) {
			throw new IOException(
					"Socket.receive(): Socket closed unexpectedly");
		} else {
		...
		// System.err.println("Rcvd " + bytesRead + " bytes via tcpip::socket: "
		// + buffer);

		return returnData;
	}

	public byte[] receiveBytes(int bufSize) throws UnknownHostException,
			IOException {
		byte[] buffer;
		int bytesRead = 0;
		int readThisTime = 0;
		...
		while (bytesRead < bufSize) {
			...
			readThisTime = inStream
					.read(buffer, bytesRead, bufSize - bytesRead);
		 ...
		 * for (int i = 0; i < bytesRead; ++i) System.err.print( " " + buffer[i]
		 * + " "); System.err.println("]");
		 */
		return buffer;
	}
